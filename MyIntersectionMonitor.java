...
public class MyIntersectionMonitor extends JFrame implements Observer {
	...
	public MyIntersectionMonitor(String title) {
		...
		fmo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					openIntersection();
				} catch (TrafficIOException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				} catch (TrafficSyntaxException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
			}
		...
		JMenuItem fmDemoMenu = new JMenu("My Demo...");
		JMenuItem fmDemoTimed = new JMenuItem("Pre-timed");
		fmDemoMenu.add(fmDemoTimed);
		JMenuItem fmDemoActuated = new JMenuItem("Fully Actuated");
		fmDemoMenu.add(fmDemoActuated);

		//fmMine.setBackground(diyColour);
		// How to run the demo I wrote myself
		fmDemoTimed.addActionListener(new ActionListener() {
		...
		fmDemoActuated.addActionListener(new ActionListener() {
	 ...
	 * @throws TrafficIOException 
	 */
	private void openIntersection() throws TrafficIOException, TrafficSyntaxException {
		File f;
	...
	private void saveSerialisedIntersection() {
		...
		try {
	        ...
	        if (chosenFile != null && userChoice == 0) 
	        {
	            ...
	            {
	            	FileOutputStream fileOut = new FileOutputStream(chosenFile.getPath());
	    	        ObjectOutputStream out = new ObjectOutputStream(fileOut);
	    	        out.writeObject(myIntersection);
	    	        out.close();
	    	        fileOut.close();
	...
	/**
	 * Load intersection from file system in serialised form.
	 */
		...
		case JFileChooser.APPROVE_OPTION:
			...
			if (f.exists() && f.isFile() && f.canRead()) {
				...
				try {
			        ...
			        myIntersection = (Intersection) in.readObject();
			        in.close();
			        fileIn.close();
